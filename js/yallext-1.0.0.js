...
var yall = function (userOptions) {
    let objectAssign = Object.assign || function (e) {
        ...
        if (element.tagName === "IMG") {
            ...
            if (env.asyncDecodeSupport === true && env.replaceWithSupport === true) {
                ...
                newImageElement.decode().then(() => {
                    ...
                    newImageElement.classList.add(options.imgLoadedClass);
                    element.replaceWith(newImageElement);
                }).catch(() => {
                    yallFlipDataAttrs(element);

                    element.onload = function (event) { event.target.classList.add(options.imgLoadedClass) };
                });
            } else {
                yallFlipDataAttrs(element);
